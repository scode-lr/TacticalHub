...
import { addOutline, createOutline, calendarOutline, flashOutline, documentTextOutline } from 'ionicons/icons';
...
export class SettingsFormsPage {
  ...
  constructor() {
    addIcons({ addOutline, createOutline, calendarOutline, flashOutline, documentTextOutline });
  }
  ...
  editForm(form: FormHeader): void {
    const { roleType, roleId } = this.navigationService.extractRoleDetails();
    this.navigationService.navigateTo([`/app/${roleType}/${roleId}/settings-forms/${form.id}`]);
  }
}
